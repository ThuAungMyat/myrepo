...
namespace SampleApi.Controllers
{
    public class ArtistController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Artist/GetArtist2")]
        public HttpResponseMessage GetArtist2(HttpRequestMessage request)
        {
            using (var ctx = new Context())
            {
                List<Artist> result = ctx.Artists.Where(a => a.IsDeleted != true).ToList();
                return request.CreateResponse<IEnumerable<Artist>>(HttpStatusCode.OK, result);
            }
